if (isServer) then {
...

	{
		...
		call {
			if (_class == "Random") exitWith { _class = cai_static_weapons call BIS_fnc_selectRandom; };
		};
		...
		if (cai_static_skills) then {

			{
				_unit setSkill [(_x select 0),(_x select 1)]
			} count cai_static_array;
		...
		cai_emplacement_units = (cai_emplacement_units + 1);
		_unit addEventHandler ["Killed",{[_this select 0, _this select 1, "civestatic"] call civ_on_kill;}];
		...
		if (!isNil "_mission") then {
			_ainum = (cai_mission_data select _mission) select 0;
			cai_mission_data select _mission set [0, (_ainum + 1)];
			_static setVariable ["civmissionclean","civstatic"];
			_static setVariable ["civmission",_mission];
			_unit setVariable ["civmission",_mission];
		};

		[_static] spawn civ_vehicle_monitor;
	...
	diag_log format ["CAI: Spawned in %1 %2",_unitnumber,_class];
