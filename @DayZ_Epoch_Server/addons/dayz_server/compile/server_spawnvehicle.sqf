private ["_player","_vehicledata","_arrow","_key","_garage","_objectUID","_class","_cid","_colour1","_inventory","_fuel","_damage","_hitpoints","_location","_dir","_worldspace","_uid","_vehiclelist","_newlist"];
_player = _this select 0;
_vehicledata = _this select 1;
_arrow = _this select 2;
_garage = _this select 3;

_objectUID = _vehicledata select 0;
_class = _vehicledata select 1;
_cid = str(_vehicledata select 2);
_colour1 = _vehicledata select 3;
_colour2 = _vehicledata select 4;
_inventory = _vehicledata select 5;
_fuel = _vehicledata select 6;
_damage = _vehicledata select 7;
_hitpoints = _vehicledata select 8;
...
_newlist = [];
{
	...
	_toremove = false;
	if ((_x select 0) == _objectUID) then { _toremove = true; };
	if (!_toremove) then {
		_data = [(_x select 0),(_x select 1),(_x select 2),(_x select 3),(_x select 4),(_x select 5),(_x select 6),(_x select 7),(_x select 8)];
