...
if(isServer) then {
	...
	while {true} do
	{
		...
		if(((_currTime - _h_startTime >= _h_missionTime) && (h_missionsrunning < wai_hero_limit)) || h_missionsrunning == 0) then { _hresult = 1; };
		if(((_currTime - _b_startTime >= _b_missionTime) && (b_missionsrunning < wai_bandit_limit)) || b_missionsrunning == 0) then { _bresult = 1; };
