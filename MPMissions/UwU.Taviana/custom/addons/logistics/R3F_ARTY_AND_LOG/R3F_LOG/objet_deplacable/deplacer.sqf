...
else
{
	...
	private ["_objet", "_est_calculateur", "_arme_principale", "_action_menu", "_azimut_canon","_posZ"];
	
	_objet = _this select 0;
	_vehicleNameText = [_objet] call MF_Tow_Get_Vehicle_Name;
	if ((count (crew _objet)) != 0) exitWith { cutText [format["Cannot pick up %1 because it has people in it.", _vehicleNameText], "PLAIN DOWN"];}; // Check if the vehicle has anyone in it
	...
	if !(isNil "_est_calculateur") then
	{
		...
		[_objet] execVM "custom\addons\logistics\R3F_ARTY_AND_LOG\R3F_ARTY\poste_commandement\deplacer_calculateur.sqf";
	}
	else
	{
		...
		else
		{
			...
			_action_menu = player addAction [("<t color=""#dddd00"">" + STR_R3F_LOG_action_relacher_objet + "</t>"), "custom\addons\logistics\R3F_ARTY_AND_LOG\R3F_LOG\objet_deplacable\relacher.sqf", nil, 5, true, true];
			...
			while {!isNull R3F_LOG_joueur_deplace_objet && alive player} do
			{
				...
				if ([0,0,0] distance (velocity player) > 25) then
				{
			...
			detach _objet;
			if ((getPos _objet select 2) < 0) then {
				_posZ = 0;
			} else {
				_posZ = ((getPosATL player) Select 2) - 0.1;
			};
			_objet setPos [getPos _objet select 0, getPos _objet select 1, _posZ];
			_objet setVelocity [0, 0, 0];
