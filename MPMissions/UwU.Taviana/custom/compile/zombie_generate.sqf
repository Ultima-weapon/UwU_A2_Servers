private ["_position","_unitTypes","_radius","_method","_agent","_cash"];
	...
	if ([_position, dayz_cantseefov, 10, dayz_cantseeDist] call _cantSee) then {
		...
		if (_rnd < 0.2) then {
			...
			if (isText _lootType) then {
				_array = [];
				if (DZE_MissionLootTable) then {
					{_array set [count _array, _x select 0]} forEach getArray (missionConfigFile >> "cfgLoot" >> getText(_lootType));
				} else {
					{_array set [count _array, _x select 0]} forEach getArray (configFile >> "cfgLoot" >> getText(_lootType));
				};
		...
		if (!isNull _agent) then {
			...
			*/
			...
			//Single Currency
			_cash = round(random 50); // number between 0 and 50
			_agent setVariable ["CashMoney",_cash ,true];
		...
		//Start behavior
		_id = [_position,_agent] execFSM "custom\compile\zombie_agent.fsm";
	};
}; 