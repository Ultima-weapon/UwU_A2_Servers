        ...
        if(!isNull player) then {
            ...
            if (_isInStripRange && {!_isTargetSelf} && {!_isTargetAlive} && {_canStripTarget} && {!_isTargetStripped} && {!DZE_TAKECLOTHES_BUSY}) then {
                if (S_PLAYER_CLOTHES < 0) then {
                    S_PLAYER_CLOTHES = player addAction [format["<t color='%1'>",DZE_COLOR_WARNING_HEX] + "Take Clothes" + "</t>", "custom\addons\clothes\takeclothes.sqf",cursorTarget,-10,false,true,"",""];
                };
            } else {
                player removeAction S_PLAYER_CLOTHES;
                S_PLAYER_CLOTHES = -1;
