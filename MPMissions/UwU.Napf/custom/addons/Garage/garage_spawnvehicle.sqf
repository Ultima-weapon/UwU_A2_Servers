...
if ((count _helipadsnear) < 1) exitWith {cutText["\n\nYou need a Helipad witin 50 meters to spawn a vehicle!","PLAIN DOWN"]};

_nearestpad = _helipadsnear select 0;
_position = getPosATL _nearestpad;

_checkkey = [player,_key] call BIS_fnc_invAdd;
if (!(_checkkey)) exitWith {cutText["\n\nYour Toolbelt is full or the same key exists on your Toolbelt.","PLAIN DOWN"]};
