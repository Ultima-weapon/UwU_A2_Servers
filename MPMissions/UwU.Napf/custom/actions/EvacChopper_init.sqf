...
while{true} do {
	...
	if ((vehicle player) == player) then {
		...
		if (!isNull cursorTarget) then {
			...
			if ((typeOf cursorTarget) isKindOf "Helicopter") then {
						...
						if (evac_vehicleKey != "0") then {
							...
							if (evac_hasKey) then {
								...
								if (s_player_makeEvacChopper < 0) then {
									s_player_makeEvacChopper = player addAction [("<t color=""#0000FF"">" + ("Set Evac-Chopper") + "</t>"),"custom\actions\SetEvacChopper.sqf",evac_cTarget,-1000,false,false,"",""];
								};
								...
								if (playerHasEvacField) then {
									...
									if ((evac_cTarget distance playersEvacField) <= 10) then {
										...
										if (s_player_clearEvacChopper < 0) then {
											s_player_clearEvacChopper = player addAction [("<t color=""#0000FF"">" + ("Clear Evac-Chopper") + "</t>"),"custom\actions\ClearEvacChopper.sqf",evac_cTarget,-1001,false,false,"",""];
										};
							...
							if (evac_needRadio == 1) then {
								...
								if (evac_call_hasRadio) then {
									...
									if (s_player_evacCall < 0) then {
										s_player_evacCall = player addAction [("<t color=""#0000FF"">" + ("Call Evac-Chopper") + "</t>"),"custom\actions\CallEvacChopper.sqf",[],-1000,false,false,"",""];
									};
