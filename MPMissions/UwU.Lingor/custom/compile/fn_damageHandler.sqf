...
// NO RANK
if(_humRank < 5000 && _humRank > -5000) then {_scale = 200;};
// HERO RANK
if(_humRank >= 5000 && _humRank < 10000) then {_scale = 200*0.95;};
if(_humRank >= 10000 && _humRank < 15000) then {_scale = 200*0.90;};
if(_humRank >= 15000 && _humRank < 25000) then {_scale = 200*0.85;};
if(_humRank >= 25000 && _humRank < 40000) then {_scale = 200*0.80;};
if(_humRank >= 40000 && _humRank < 60000) then {_scale = 200*0.75;};
if(_humRank >= 60000 && _humRank < 90000) then {_scale = 200*0.70;};
if(_humRank >= 90000 && _humRank < 130000) then {_scale = 200*0.65;};
if(_humRank >= 130000 && _humRank < 185000) then {_scale = 200*0.60;};
if(_humRank >= 185000 && _humRank < 250000) then {_scale = 200*0.55;};
if(_humRank >= 250000) then {_scale = 200*0.50;};
// BANDIT RANK
if(_humRank <= -5000 && _humRank > -10000) then {_scale = 200*1.05;};
if(_humRank <= -10000 && _humRank > -15000) then {_scale = 200*1.10;};
if(_humRank <= -15000 && _humRank > -25000) then {_scale = 200*1.15;};
if(_humRank <= -25000 && _humRank > -40000) then {_scale = 200*1.20;};
if(_humRank <= -40000 && _humRank > -60000) then {_scale = 200*1.25;};
if(_humRank <= -60000 && _humRank > -90000) then {_scale = 200*1.30;};
if(_humRank <= -90000 && _humRank > -130000) then {_scale = 200*1.35;};
if(_humRank <= -130000 && _humRank > -185000) then {_scale = 200*1.40;};
if(_humRank <= -185000 && _humRank > -250000) then {_scale = 200*1.45;};
if(_humRank <= -250000) then {_scale = 200*1.50;};
if (_damage > 0.4) then {
	...
	if ((isPlayer _source) && !(player == _source)) then {
		if (_unitIsPlayer) then {
			_vehicle = typeOf (vehicle _source); 
			if ((getText (configFile >> "CfgVehicles" >> _vehicle >> "vehicleClass")) in ["CarW","Car","CarD","Armored","Ship","Support","Air","ArmouredW","ArmouredD","SupportWoodland_ACR","StaticWeapon","Motorcycle","Truck","Wheeled_APC","Tracked_APC","Tank"]) then {
				...
				if ((typeName _source) != "STRING") then {
					_weapon_dmg = gettext (configFile >> 'cfgWeapons' >> (currentWeapon _source) >> 'displayName');
					_weapon_img = gettext(configFile >> 'cfgWeapons' >> (currentWeapon _source) >> 'picture');
				} else {
					_weapon_dmg = "nil";
			...
			if (isPlayer _source) then {
				player setVariable["AttackedBy", _source, true];
				player setVariable["AttackedByName", (name _source), true];
				player setVariable["AttackedByWeapon", _weapon_dmg, true];
				player setVariable["AttackedFromDistance", _distance, true];
				player setVariable["AttackedByWeaponImg", _weapon_img, true];
			};
		};
		_scale = _scale + 800;
		if (_isHeadHit) then {
