// included compiles
call compile preprocessFileLineNumbers "custom\addons\bike\wrapper.sqf";
call compile preprocessFileLineNumbers "custom\addons\bike\functions.sqf";

DZE_DEPLOYABLE_VERSION = "2.5.1";

//diag_log text format["BIKE: loading version %1 ...",DZE_DEPLOYABLE_VERSION];

player_deploy = compile preprocessFileLineNumbers "custom\addons\bike\player_deploy.sqf";

// inflate deployables
DZE_DEPLOYABLES = [];
{
    private["_class","_distance","_deployables","_packDist","_packOthers","_clearCargo","_permanent","_damage","_needNear","_parts","_requirePlot","_enableSim","_road","_reqRank"];
    _class       = _x select 0;
    ...
    _parts       = _x select 12;
	_reqRank	 = _x select 13;
    {
        DZE_DEPLOYABLES set [count DZE_DEPLOYABLES,[_class,_distance,_packDist,_damage,_packOthers,_clearCargo,_permanent,_x,_needNear,_parts,_requirePlot,_enableSim,_road,_reqRank]];
...
[] spawn {
    ...
    {DZE_CLICK_ACTIONS = DZE_CLICK_ACTIONS + [[(_forEachIndex call getDeployableKitClass),format["Deploy %1",(_forEachIndex call getDeployableDisplay)],format["%1 execVM 'custom\addons\bike\deploy.sqf';",_forEachIndex],"true"]];} forEach DZE_DEPLOYABLES;
    DZE_DEPLOYING      = false;
    DZE_PACKING        = false;
            ...
            {   
                ...
                if(!(isNull _cursorTarget)
                    ...
                    if ((_forEachIndex call getActionId) < 0) then {
                        [_forEachIndex,player addaction[format["<t color='%1'>",DZE_COLOR_PRIMARY_HEX] + format["Pack %1",(_forEachIndex call getDeployableDisplay)] + "</t>","custom\addons\bike\pack.sqf",[_forEachIndex,_cursorTarget],0,false,true,"", ""]] call setActionId;
                    };
                } else {
